...
public class ImageRGB {
  ...
  // (o f.bmp si falta la extension en el nombre)
	...
	try{
	  ...
	  inBMP.skipBytes(28); // salta los datos no utiles de la cabecera
	  // crea y rellena img
  ...
  // efecto: indica si se ha guardado el archivo
    ...
    try{
      ...
      int tamanoRelleno =(4-((ancho*3) % 4))%4; // numero de bytes de relleno al final de cada linea
      escribeEntero32bits(outBMP,altura*(ancho*3+tamanoRelleno)+54); // Tamanioo del fichero
      ...
      outBMP.write(24); outBMP.write(0); // Numero de bits por pixel
      escribeEntero32bits(outBMP,0); // sin compresion
      escribeEntero32bits(outBMP,altura*(ancho*3+tamanoRelleno)); // tamanioo de la imagen en bytes
      escribeEntero32bits(outBMP,2851); escribeEntero32bits(outBMP,2851); // resolucion en ancho x alto
  ...
  // efecto: escribe n en la salida sobre 4 bytes segun el formato littel indian
    try {
      salida.write((n) & 0xFF);
  ...
  // resultado : el pixel en posicion (i,j)
  ...
  // requisito : 0<=i<this.altura() et 0<=j<this.ancho()
  // efecto : cambia por pix el pixel en posicion (i,j)
